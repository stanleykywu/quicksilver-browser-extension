...
Additionally, the `chromium/` folder contains the publicly available web extension. Note that by default, the WASM bindings are not included in the `chromium/` folder. To install the bindings, run `./scripts/build.sh web` (see [#building-quicksilver]).
